...
describe('DBeatsFactory', function () {
    ...
    describe('Deployment', function () {
        ...
        it('Should assign role to user', async function () {
            ...
            expect(
                await dBeatsFactory.hasRole(Admin_role, addr1.address),
            ).to.equal(true)
        })
    ...
    describe('Create NFT', function () {
        ...
        it('Should create new NFT contract', async function () {
            ...
            const newNftAddress = await dBeatsFactory.nftsByCreator(
                addr1.address,
                0,
            )
            expect(
                await dBeatsFactory.nftsByCreator(addr1.address, 0),
            ).to.equal(newNftAddress)
        })
        ...
        it('Should mint new NFT to a new user', async function () {
            ...
            await connectUserToContract.mint(addr2.address, 1, {
                value: mintPrice,
            })
        ...
        it('Should return the balance of the NFT contract after minting', async function () {
            ...
            const contractBalance =
                await ethers.provider.getBalance(newNftAddress)
