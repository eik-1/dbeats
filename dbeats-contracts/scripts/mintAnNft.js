...
const nftAddress="0x95218e3B7Abce255167919C714AEB54514F7b882"
const provider = new ethers.JsonRpcProvider(process.env.SEPOLIA_RPC_URL)
const wallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider)
const signer = wallet.signer
const contract = new ethers.Contract(nftAddress, abi, provider)
const contractwithsigner = new ethers.Contract(nftAddress, abi, wallet)
...
async function mintAnNft() {
    ...
    const tx = await contractwithsigner.mint(publicKey, 1, {value: mintPrice})
    await tx.wait()
    console.log(tx.hash)
}
